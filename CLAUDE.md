...
- `MinimaxEngine` — public facade: owns the long-lived transposition table, time manager, move-ordering state, and the per-search context; the search itself lives in `PVSearcher`. Public API: `FindBestMove`, `SetTimeLimit` (changes difficulty mid-game, TT persists), `NodesSearched` / `LastCompletedDepth` / `Stats` (UI snapshot: depth, nodes, nps, TB hits, score). Ctor: `MinimaxEngine(TimeSpan? timeLimit = null, int? maxDepth = null, bool legacyEval = false, bool legacySearch = false, bool useTablebase = true, int contempt = 30, long? maxNodes = null)` — the legacy flags disable the P3 eval terms / P4 search features for A/B strength testing; `useTablebase` disables tablebase probing (the tactical suite pins pure-search behavior); `contempt` is the draw-avoidance bias in centipawns (0 = off)
